...
namespace ICP.Host.ApiTest.Services
{
 ...
 bool isValid = _rsaCryptoHelper.VerifySignDataWithSha256(stringResult, resultSignature);
        ...
        private MockSetting GetMockSetting(string env)
        {
            ...
            if (settings != null && settings.ContainsKey(env))
            {
                return settings[env];
            }
            else
            {
                // 可以根據需求決定如何處理找不到設置的情況
                // 例如，返回一個預設的 MockSetting，或者拋出一個異常
                return null; // 或者返回預設值
            }
        }
        ...
        {
            // 獲取現有的設置，如果為 null，則初始化一個新的字典
            var settings = GetAllMockSettings() ?? new Dictionary<string, MockSetting>();
        ...
        private NewStruct CallCertificateApi(string action, long certId, string serverPublicKey, string clientPrivateKey, object obj, string certHeaderName)
        {
    ...
    internal struct NewStruct
    {
        ...
        public NewStruct(string content, string signature)
        {
            Content = content;
            Signature = signature;
        }
        ...
        {
            int hashCode = 928454079;
            hashCode = hashCode * -1521134295 + EqualityComparer<string>.Default.GetHashCode(Content);
            hashCode = hashCode * -1521134295 + EqualityComparer<string>.Default.GetHashCode(Signature);
            return hashCode;
        }

        public void Deconstruct(out string content, out string signature)
        {
            content = Content;
            signature = Signature;
        }

        public static implicit operator (string Content, string Signature)(NewStruct value)
        {
            return (value.Content, value.Signature);
        }

        public static implicit operator NewStruct((string Content, string Signature) value)
        {
            return new NewStruct(value.Content, value.Signature);
        }
    }
}